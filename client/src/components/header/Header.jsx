...
export default function Header({
  ...
  toggle
    ? (headerClassArr[1] = "header_open_height")
    : (headerClassArr[1] = "header_closed_height");
    ...
    <header className={headerClassArr.join(" ")}>
      <nav className="header__nav">
        <div className="header__menu">
          ...
          <img
            src={toggleImg}
            alt="открыть / закрыть навигацию"
            onClick={() => toggleHandler()}
            className="header_menu_toggle"
          />
        </div>
        <ul className={menuClassArr.join(" ")}>
          <li className="menu__item">
            <a
              className="anchor header_menu_anchor"
